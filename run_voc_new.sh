...
#SBATCH --output=./0923/test_%j_%%Y-%%m-%%d-%%H-%%M-%%S.txt
...
do
    python3 -m tools.ckpt_surgery -d voc -m init -s 0 \
        "dataset/check/voc/1726356812/base1/model_final.pth"
    for seed in `seq 1`
    do
        ...
        do
            python3 main.py --num-gpus $NGPU --config-file configs/voc/fsod.yaml \
                ...
                MODEL.WEIGHTS "dataset/check/voc/1726356812/base1/model_final-fsod.pth" \
