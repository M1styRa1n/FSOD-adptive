...
class FsodRes5ROIHeads(Res5ROIHeads):
    def __init__(self, cfg, input_shape):
        super(FsodRes5ROIHeads, self).__init__(cfg, input_shape)
    ...
    def forward(self, features: Dict[str, torch.Tensor], proposals: List[Instances], targets=None):
        ...
        if self.training:
            ...
            # ??:???????logits,???
            with torch.no_grad():
                teacher_logits, _ = self.teacher_model.roi_heads.box_predictor(box_features.mean(dim=[2, 3]))

            student_logits = predictions[0]  # ????logits

            # ??????
            distillation_loss = F.kl_div(
                F.log_softmax(student_logits / self.box_predictor.distillation_temp, dim=-1),
                F.softmax(teacher_logits / self.box_predictor.distillation_temp, dim=-1),
                reduction='batchmean'
            ) * (self.box_predictor.distillation_temp ** 2)

            # ????? = ???? + ????
            total_loss = 0.7 * losses["loss_cls"] + 0.3 * distillation_loss  # 0.3???????
